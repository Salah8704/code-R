...
export async function GET() {
  ...
  let state = "IDLE";
  let nextAction = { label: "Commencer ma premiÃÂ¨re sÃÂ©rie", href: "/quiz", locked: false };
  let coachMessage = "Bienvenue ! CommenÃÂ§ons par ÃÂ©valuer ton niveau.";
  ...
  if (pauseStatus.forced) {
    ...
    coachMessage = "Tu es prÃÂªt ÃÂ  passer l'examen ! Fais un dernier examen blanc.";
  } else if (activeBlock && activeBlock.seriesCompleted < 2) {
    state = "IN_BLOCK";
    const serieNum = activeBlock.seriesCompleted + 1;
    nextAction = { label: `Faire la sÃÂ©rie ${serieNum}/2`, href: "/quiz", locked: false };
    coachMessage = `Tu es en plein bloc ${activeBlock.blockNumber}. Encore une sÃÂ©rie pour le terminer.`;
  } else if (totalSeries > 0) {
    state = "BLOCK_DONE";
    nextAction = { label: "DÃÂ©marrer un nouveau bloc", href: "/quiz", locked: false };
    coachMessage = "Excellent ! Tu as terminÃÂ© ce bloc. PrÃÂªt pour le suivant ?";
  }
